...
var FeatureGateAnalyzer = &analysis.Analyzer{
	Name: "featuregate",
	Doc:  "validates featuregates across kubernetes files",
	Run:  run,
}
...
func extractFeatureInfo(spec *ast.ValueSpec) *featureInfo {
	...
	if !ok || basicLit.Kind != token.STRING {
		logger.Printf("spec.Values[0] is not a string literal: %T", spec.Values[0])
		return nil
	}
	...
	if spec.Doc != nil {
		...
		logger.Printf("All comments for %s:\n%s", name, allComments)

		// Check for alpha/beta/GA
		if strings.Contains(allComments, "alpha:") {
			info.PreRelease = "Alpha"
		} else if strings.Contains(allComments, "beta:") {
			info.PreRelease = "Beta"
		} else if strings.Contains(allComments, "GA:") {
			info.PreRelease = "GA"
		}

		// Infer Default based on PreRelease
		switch info.PreRelease {
		case "Alpha":
			info.Default = false
		case "Beta", "GA":
			info.Default = true
		...
		// Check for explicit Default override
		if strings.Contains(allComments, "Default: false") {
			info.Default = false
		} else if strings.Contains(allComments, "Default: true") {
			info.Default = true
		}

		logger.Printf("Inferred PreRelease: %s, Default: %v", info.PreRelease, info.Default)
	} else {
